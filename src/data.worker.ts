...
scope.onmessage = async ({ data: request }) => {
  try {
    ...
    engine = null;
    const bytes = typeof request.source === "string" ? new Blob([request.source]).size : request.source.size;
    if (bytes > MAX_FILE_BYTES) throw new Error("This import exceeds the 256 MiB limit. Split the file into smaller parts.");
    ...
    await parseInput(request.source, request.delimiter, result => {
      ...
      for (const row of result.data) {
        cells += row.length;
        if (cells > MAX_CELLS || row.length > MAX_COLUMNS) throw new Error("The input exceeds 5 million cells or 1,000 columns. Split it into smaller parts.");
        if (rows.length && row.some(cell => cell.trim()) && row.length !== rows[0].length) ragged = true;
        rows.push(row);
      }
    ...
    if (!rows.some(row => row.some(cell => cell.trim()))) throw new Error("The input contains no non-empty cells.");
    engine = new DataEngine(rows);
    const warning = [
      detectedWarning ? "Delimiter could not be detected; comma was used. Choose a delimiter if the columns look incorrect." : "",
      ragged ? "Some records have different numbers of fields. Missing cells are shown as blank." : "",
    ].filter(Boolean).join(" ");
    send({ type: "loaded", rows: rows.length, warning });
  } catch (error) {
    send({ type: "error", message: error instanceof Error ? error.message : "Processing failed. Try a smaller file." });
  }
};
