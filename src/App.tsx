...
function App() {
  ...
  return (
    <main className={`${styles.drop} ${isDragging ? styles.dragging : ""}`}>
      ...
      {dataset.loading && (
        <div role="status">
          <p>Loading… {dataset.progress.toLocaleString("en-AU")} records parsed.</p>
          <button onClick={dataset.clear}>Cancel import</button>
        </div>
      )}
      {dataset.warning && (
      ...
      {dataset.loaded && (
        <>
          ...
          <div className={styles.controls}>
            <label>
              <input
                ...
                onChange={() => {
                  setSettings((current) => ({ ...current, headers: !current.headers }));
                  setPageIndex(0);
                  setSort(null);
                }}
              />
            ...
            <label>
              <input
                ...
                checked={phrase}
                onChange={(event) => {
                  setPhrase(event.target.checked);
                  setPageIndex(0);
                }}
              />
              Match phrase
            ...
            <label>
              <input
                ...
                checked={skipEmpty}
                onChange={(event) => {
                  setSkipEmpty(event.target.checked);
                  setPageIndex(0);
                }}
              />
              Hide empty records
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  setSortMode(mode);
                  setSort((current) => (current ? { ...current, mode } : null));
                  setPageIndex(0);
                }}
              >
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => setDelimiterWithCopy(copyDelimiter(event.target.value))}
              >
                <option value={"\t"}>Tab</option>
                <option value=",">Comma</option>
              </select>
            ...
            <label>
              <input
                ...
                onChange={(event) => setSimplifyNumbers(event.target.checked)}
              />
          ...
          <p role="status">
            {stale || !page
              ? "Updating results…"
              : `${page.loaded.toLocaleString("en-AU")} loaded records · ${page.matched.toLocaleString("en-AU")} matches`}
          </p>
          {page && (
            <>
              <nav className={styles.controls} aria-label="Result pages">
                <button disabled={stale || page.page === 0} onClick={() => setPageIndex(0)}>
                  First
                </button>
                <button
                  disabled={stale || page.page === 0}
                  onClick={() => setPageIndex(page.page - 1)}
                >
                  Previous
                </button>
                <span>
                  Page {page.page + 1} of {pages}
                </span>
                <button
                  disabled={stale || page.page + 1 >= pages}
                  onClick={() => setPageIndex(page.page + 1)}
                >
                  Next
                </button>
                ...
                {page.pageSize < pageSize && (
                  <span>Limited to {page.pageSize} rows for this wide file.</span>
                )}
              </nav>
              {!page.matched ? (
                <p>No matching records. Try a different filter or show empty records.</p>
              ) : (
                <>
                  <p className={styles.rowInfo}>
                    Source record numbers stay unchanged when searching or sorting. Select a cell to
                    copy its full value; select its record number to copy the row. Values over 500
                    characters are previewed.
                  </p>
                  <div className={styles.tableScroll} aria-busy={stale}>
                    <ResultsTable
                      page={page}
                      delimiter={delimiterWithCopy}
                      simplifyNumbers={simplifyNumbers}
                      onCopy={copy}
                      onSort={handleSort}
                    />
                  </div>
      ...
      <p className={styles.privacy}>
        All loading and processing happen within your browser. No file data is sent to a server.
      </p>
