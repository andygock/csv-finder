...
const ResultsTable = memo(function ResultsTable({ page, delimiter, simplifyNumbers, onCopy, onSort }: ResultsTableProps) {
  ...
  const highlight = (cell: string) => {
    ...
    return <>
      {pattern
        ? preview.split(pattern).map((part, i) => i % 2
  ...
  return <table className={styles.table}>
    ...
    <thead>
      <tr>
        ...
        {page.headings.map((heading, column) => {
          ...
          return <th key={column} scope="col" aria-sort={direction}>
    ...
    <tbody>
      {page.rows.map(row => <tr key={row.id}>
        <th scope="row">
          <button className={styles.cellButton} aria-label={`Copy source record ${row.id + 1}`}
        ...
        {page.headings.map((_, column) => {
          ...
          return <td key={column}>
            <button className={styles.cellButton}
              title={`Copy record ${row.id + 1}, column ${column + 1}`}
              aria-label={cell ? undefined : `Copy empty cell, record ${row.id + 1}, column ${column + 1}`}
