...
test("settings reject invalid types and survive unavailable or full storage", () => {
  for (const value of [null, "broken", "null", '"false"', '{"headers":"false"}', '{"headers":0}']) {
    assert.deepEqual(readSettings(() => ({ getItem: () => value })), { headers: true });
  }
  assert.deepEqual(readSettings(() => ({ getItem: () => '{"headers":false}' })), { headers: false });
  ...
  assert.doesNotThrow(() => writeSettings({ headers: false }, () => ({ setItem() { throw new Error("Quota exceeded"); } })));
});

test("search shortcuts preserve modifier keys, controls, composition and dialogs", () => {
  const key = { key: "a", ctrlKey: false, metaKey: false, altKey: false, isComposing: false, defaultPrevented: false };
